...
class PolylineEntity(BaseEntity):
  ...
  def hit_test(self, p: Point, tolerance: float = 3.0) -> bool:
    ...
    for a, b in zip(self.points[:-1], self.points[1:]):
      if is_point_near_segment(p, a, b, tolerance):
        return True
    return False

  def move(self, dx: float, dy: float) -> None:
    self.points = [Point(pt.x + dx, pt.y + dy) for pt in self.points]

  def to_dict(self) -> Dict[str, Any]:
    base = super().to_dict()
    base.update({
      "points": [{"x": pt.x, "y": pt.y} for pt in self.points],
    })
    return base
...
def entity_from_dict(data: Dict[str, Any]) -> BaseEntity:
  ...
  if etype == "PolylineEntity":
    pts = [Point(float(p["x"]), float(p["y"])) for p in data.get("points", [])]
